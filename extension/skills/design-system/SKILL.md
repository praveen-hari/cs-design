...
Check if `.designs/` folder exists in the workspace. If not, use the `#cs-design-init` tool to create it:

**Tool:** `cs-design_init` — Creates `.designs/` folder with DESIGN.md, project.json, and tokens.css.
...
Before creating a DESIGN.md, use the `#cs-design-spec` tool to get the full format specification.

**Tool:** `cs-design_getSpec` — Returns the YAML schema, token types, section order, and validation rules.

The specification defines the exact format. **Always follow it.**
...
After writing the DESIGN.md file, run validation and lint:

1. **Tool:** `cs-design_validate` — Checks structural validity and reports errors/warnings.
2. **Tool:** `cs-design_lint` — Deep lint for WCAG contrast, broken refs, orphaned tokens.
3. **Tool:** `cs-design_exportTokens` with format `css` — Generates `.designs/tokens.css`.
...
### Edit Step 4 — Validate and confirm

After editing, validate the changes:

1. **Tool:** `cs-design_validate` — Check the edited file is still valid.
2. **Tool:** `cs-design_lint` — Check for new lint issues.
3. **Tool:** `cs-design_exportTokens` with format `css` — Re-export tokens.css so screens update.
