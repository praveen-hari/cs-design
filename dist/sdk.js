...
function generateCss(yaml2) {
  const lines = ["/* Light theme (default) */", ":root {"];
  if (yaml2.colors) {
    lines.push("  /* Colors */");
    for (const [key, value] of Object.entries(yaml2.colors)) {
  ...
  if (yaml2.typography) {
    ...
    for (const [key, token] of Object.entries(yaml2.typography)) {
  ...
  if (yaml2.rounded) {
    ...
    for (const [key, value] of Object.entries(yaml2.rounded)) {
  ...
  if (yaml2.spacing) {
    lines.push("  /* Spacing */");
    for (const [key, value] of Object.entries(yaml2.spacing)) {
      lines.push(`  --space-${key}: ${value};`);
    }
    lines.push("");
  }
  lines.push("}");
  const darkColors = yaml2["colors-dark"];
...
function generateTailwind(yaml2) {
  const theme = {};
  if (yaml2.colors) {
    const colors = { ...yaml2.colors };
    const darkColors = yaml2["colors-dark"];
    if (darkColors) {
  ...
  if (yaml2.rounded) {
    theme.borderRadius = { ...yaml2.rounded };
  }
  if (yaml2.spacing) {
    theme.spacing = { ...yaml2.spacing };
  }
  return `/** @type {import('tailwindcss').Config['theme']} */
export default ${JSON.stringify(theme, null, 2)};
`;
}
function generateJson(yaml2) {
  const tokens = {};
  if (yaml2.colors) {
    for (const [key, value] of Object.entries(yaml2.colors)) {
